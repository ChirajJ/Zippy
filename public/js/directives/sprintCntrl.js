(function(){
    ...
    var backlogCntrl = function($scope, $http, $window, $uibModal, $rootScope){
        ...
        $scope.loadSprData = function(){
            console.log('loading...');
            
            var n = $scope.sprintNo;
            
            $http.get('/userStory/sprint/'+n).then(function(resp){
                
                console.log('Sprint');
                console.log(resp);
                //$scope.data = resp.data[0].title;
                if(!resp.data.error){
                    $rootScope.$broadcast('db-sprint-stories-recieved', 
                                 {data: resp.data.stories});
                }
            });
        ...
        $scope.$on('db-sprint-stories-recieved', function(evt, data){
            
            console.log('Recieved');
            console.log(data.data);
            $scope.sprStories = data.data;
            $scope.sprintFlag = true;
    ...
    app.controller('sprintCntrl', ['$scope', '$http', '$window', '$uibModal', '$rootScope', backlogCntrl]);
    
    app.controller('sprintPopUp', ['$scope', '$http', '$window', '$uibModalInstance', '$rootScope', popUp]);
